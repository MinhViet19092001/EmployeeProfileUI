<template>
 <div class="content-organization-unit">
        <div class="container-org w-full h-full">
            <ToolbarGridBase 
                :widthInputSeach="320" 
                :placeHolderInputSearch="'Tìm kiếm vai trò người dùng'"
                @onSearch="onSearch"
            >
            </ToolbarGridBase>
            <div class="grid-org">
...
export default {
    ...
    methods: {
        ...
        async getDataSource(){
            ...
            //Lấy ra dataSource
            await UserRoleAPI.getDataPaging(this.paramPaging).then(
                (res) => {
                    if(res){
        ...
        showInfoDetail(e){
            this.$router.push({ name: "userRoleDetail", params: { id: e?.data?.UserRoleID } });
        },
        changePageSize(e){
        ...
        editItem(userRoleID){
            if(this.roleUserRole?.includes("Edit")){
        ...
        deleteItem(userRole){
            if(this.roleUserRole?.includes("Delete")){
